 ...
 * 
 */
package com.iflytek.voicecloud.webapi_tts;
...
public class WebTtsUtil {
	
	private static final Logger logger = LoggerFactory.getLogger(WebTtsUtil.class);
	
	@Autowired
	private BaseFrameworkConfig config;
	
	// 合成webapi接口地址
	private static String WEBTTS_URL = "https://api.xfyun.cn/v1/service/v1/tts";
	...
	private static  String API_KEY = "e24ffb6e714013fa778f30d2834a635b";
	
	private static String videoDialogPath="";
	...
	static {
        try {
        		...
        		logger.info("WebTtsUtil VideoDialogTtsName{}",VideoDialogTtsName);
        } catch (Exception e) {
	...
	public static String getWebTtsVoiceUrlByText(String voiceResultText) throws IOException {
		...
		if ("audio/mpeg".equals(resultMap.get("Content-Type"))) { // 合成成功
			if ("raw".equals(AUE)) {
				...
				//String mainget
				// videoDialogPath VideoDialogTtsName
				//String  videoDialogTtsPath = videoDialogPath + VideoDialogTtsName + VideoUtils.FILE_SEPARATOR;
				//本地先不存储 
				//FileUtil.save(videoDialogTtsPath, resultMap.get("sid") + ".mp3", (byte[]) resultMap.get("body"));
	...
	public static void main(String[] args) throws IOException {
		...
		if ("audio/mpeg".equals(resultMap.get("Content-Type"))) { // 合成成功
			if ("raw".equals(AUE)) {
				FileUtil.save("/Users/harry/out/", resultMap.get("sid") + ".wav", (byte[]) resultMap.get("body"));
				System.out.println("合成 WebAPI 调用成功，音频保存位置：resource\\" + resultMap.get("sid") + ".wav");
			} else {
				//FileUtil.save("/Users/harry/out/", resultMap.get("sid") + ".mp3", (byte[]) resultMap.get("body"));
				InputStream inputStream = new ByteArrayInputStream((byte[]) resultMap.get("body")); 
