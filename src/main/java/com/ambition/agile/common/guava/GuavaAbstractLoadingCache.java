...
public abstract class GuavaAbstractLoadingCache <K, V> {    
    ...
    private TimeUnit timeUnit = TimeUnit.SECONDS;   //MINUTES;   //时间单位（分钟 秒）    
     ...
     */    
    protected abstract V fetchData(K key,K value);    
    ...
    protected void remove(K key) throws ExecutionException {    
        getCache().invalidate(key);
    }
    
    protected void removeKes(List<K> keys) throws ExecutionException {    
        getCache().invalidateAll(keys);
    }
    
    protected void removeAll() throws ExecutionException {    
